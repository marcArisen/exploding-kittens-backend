...
describe('Player system', () => {
  ...
  test('Discard from my hand by card index', () => {
    me.addCardToHand(new NumberCard('Cat'));
    let handLength = me.getHandLength();
    let cardIndex = 0;
    me.removeCardByIndex(cardIndex);
    expect(me.getHandLength()).toEqual(handLength - 1);
  });
